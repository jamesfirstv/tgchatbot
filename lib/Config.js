...
var Config = class {
  ...
  private logChk(g) {
    ...
    chkBool(data, cbg, cbe) {
      if (typeof data == 'boolean') {cbg(data); return}
      if (typeof data == 'string') {
        if (data == 'true') {cbg(true); return}
        if (data == 'false') {cbg(false); return}
      }
      cbe()
    }

    chkBool(this.config_json.logs,    (data) => this.logs     = data, throwErr('logs'))
    chkBool(this.config_json.conlogs, (data) => this.conlogs  = data, throwErr('conlogs'))
  }
}
